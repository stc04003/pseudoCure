fitPHMC <- function(X1, X2, time, status, t0, control) {
    ...
    fit1 <- fit2 <- NULL
    if (!is.null(X1)) fit1 <- fitPHMC1(X1, time, status, control, cure, curei)
    if (!is.null(X2)) fit2 <- fitPHMC2(X2, time, status, t0, control, cure, curei, S, Si)
...
fitPHMC2 <- function(X2, time, status, t0, control,
  ...
  SSi <- n * (S - cure) / (1 - cure) - c((n - 1) * t((t(Si) - curei) / (1 - curei)))
  Fi <- 1 - SSi
